...
boolean Chess::movePromotion(short attackCol, short piece, short fromRow, short fromCol) {
  short attackRow = 0;
  if (whosTurn == 1) {
    attackRow = 7;
  }
  if (moveAttacker(attackRow, attackCol, pawn, fromRow, fromCol)) {
    board[attackRow][attackCol] = whosTurn * piece;
    return true;
  }
  return false;
}
